function init() {
    ...
    if (e && player) {
        ...
        let html = `<iframe src="${t.link}?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" allowfullscreen allowtransparency allow="autoplay"></iframe>`;

        player.insertAdjacentHTML('afterbegin', html)

        let o = Plyr.setup("#player", t)[0];
        ...
        void 0 !== t.poster && (o.poster = t.poster),
            ...
            o.on("ready", (e) => {
                ...
                setTimeout(() => {
                    ...
                    for (var e = document.querySelectorAll(".plyr__control, .plyr--full-ui input[type=range], .plyr__control--overlaid"), o = 0; o < e.length; o++) e[o].style.color = t.progressBarColor;

                    const r = document.querySelectorAll(".plyr--full-ui input[type=range], .plyr__volume input[type=range]");

                    for (let e = 0; e < r.length; e++) r[e].style.setProperty("--plyr-range-thumb-background", t.controlBarColor);
                    ...
                    if ((l.styleSheet ? (l.styleSheet.cssText = n) : l.appendChild(document.createTextNode(n)), document.head.appendChild(l), t.muted)) {

                        let e = '<div class="video-sound-overlay">';

                        (e += '<div class="unmute-button">'),

                            void 0 !== t.mutedImageUrl && t.mutedImageUrl && (e += `<img src="${t.mutedImageUrl}" style="width:30%" alt="Click To Turn On Sound">`),

                            (e += "</div>"),

                            (e += "</div>"),

                            document.querySelector(".plyr__video-embed").insertAdjacentHTML("beforeend", e);
...
const Player = {
    ...
    run: () => {
        ...
        (async () => {
            ...
            await load_scripts(["https://cdn.plyr.io/3.7.3/plyr.js", "https://code.jquery.com/jquery-3.6.0.js", "https://cdnjs.cloudflare.com/ajax/libs/prefixfree/1.0.7/prefixfree.min.js"]);
